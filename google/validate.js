...
const EXPIRE_BUFFER_TIME = 600000; // 10min
const jwtClients = [];

function requestPublishingApi(err, tokens, opt) {
  const { jwtClient, reqPurchaseData, purchaseData, callback, timeoutTimer } = opt;
  if (err) {
  ...
  requestGoogleAPI(params, (err2, bodyObj) => {
    ...
    if (err2) {
      jwtClients[purchaseData.packageName] = null;
...
function validate(reqPurchaseData, callback) {
  ...
  const jwtCache = jwtClients[purchaseData.packageName];
  if (!jwtCache || !jwtCache.credentials
     || !jwtCache.credentials.expiry_date
     || Date.now() > (jwtCache.credentials.expiry_date - EXPIRE_BUFFER_TIME)
     ) {
    ...
    jwtClient.authorize((err, tokens) => {
      if (!err) jwtClients[purchaseData.packageName] = jwtClient;
      const opt = { jwtClient, reqPurchaseData, purchaseData, callback, timeoutTimer };
      requestPublishingApi(err, tokens, opt);
    });
  } else {
    const opt = { jwtClient: jwtCache, reqPurchaseData, purchaseData, callback, timeoutTimer };
    requestPublishingApi(null, jwtCache.credentials, opt);
  }
}
